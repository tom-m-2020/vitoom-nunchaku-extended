...
void gemm_w8a16(torch::Tensor input,
    ...
    TORCH_CHECK(out.is_contiguous(), "gemm_w8a16: `out` must be contiguous");
    auto wscales_c = wscales.contiguous();
    auto bias_c = bias.has_value() ? bias.value().contiguous() : torch::Tensor{};

    nunchaku::kernels::gemm_w8a16(from_torch(input_c),
                                  from_torch(qweight_c),
                                  from_torch(out),
                                  from_torch(wscales_c),
                                  bias_c.defined() ? from_torch(bias_c) : Tensor{});
}

void quantize_w8a16_wgt(torch::Tensor input,
                        torch::Tensor output,
                        torch::Tensor wscales) {
    TorchOpContext ctx;
    auto input_c = input.contiguous();
    TORCH_CHECK(output.is_contiguous(), "quantize_w8a16_wgt: `output` must be contiguous");
    TORCH_CHECK(wscales.is_contiguous(), "quantize_w8a16_wgt: `wscales` must be contiguous");

    nunchaku::kernels::quantize_w8a16_wgt(from_torch(input_c), from_torch(output), from_torch(wscales));
}
