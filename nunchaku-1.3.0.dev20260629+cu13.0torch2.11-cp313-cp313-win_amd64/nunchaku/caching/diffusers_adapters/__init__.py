...
def apply_cache_on_pipe(pipe: DiffusionPipeline, *args, **kwargs):
    ...
    **kwargs
    ...
    ------
    ValueError
        If the pipeline type is not supported (doesn't start with "Flux", "Sana", "Qwen", "StableDiffusionXL", or "Chroma").
    AssertionError
    ...
    .. warning::
       Only pipelines with class names starting with ``Flux``, ``Sana``, ``Qwen``, ``StableDiffusionXL``, or ``Chroma`` are supported.
    """
    ...
    elif pipe_cls_name.startswith("StableDiffusionXL"):
        from .sdxl import apply_cache_on_pipe as apply_cache_on_pipe_fn
    elif pipe_cls_name.startswith("Chroma"):
        from .chroma import apply_cache_on_pipe as apply_cache_on_pipe_fn
    elif pipe_cls_name.startswith("Qwen"):
        from .qwenimage import apply_cache_on_pipe as apply_cache_on_pipe_fn
    else:
