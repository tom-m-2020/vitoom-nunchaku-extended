...
class NunchakuFeedForward(FeedForward):
    ...
    **kwargs :
    ...
    def __init__(self, ff: FeedForward, linear_cls=SVDQW4A4Linear, **kwargs):
        super(FeedForward, self).__init__()
        self.net = _patch_linear(ff.net, linear_cls, **kwargs)
        # For int4, shift the activation of mlp_fc2 to make it unsigned
        if isinstance(self.net[2], SVDQW4A4Linear):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Fallback to original implementation. This is also the W8A16 path,
        # because fused_gelu_mlp is specialized for SVDQ W4 linears.
