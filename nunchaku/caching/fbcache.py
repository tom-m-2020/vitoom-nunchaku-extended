...
def _qualify_buffer_name(name: str, cache_key_prefix: str | None = None) -> str:
    if cache_key_prefix is None or cache_key_prefix == "":
        return name
...
def configure_cache_context(
    ...
    *,
    ...
    """
    Initialize cache context metadata for one pipeline invocation.
    """
    cache_ctx.fbcache_num_inference_steps = None if num_inference_steps is None else max(int(num_inference_steps), 0)
    cache_ctx.fbcache_num_branches = max(int(num_branches), 1)
    cache_ctx.fbcache_call_idx = 0
    cache_ctx.fbcache_current_step = 0
    cache_ctx.fbcache_current_branch_idx = 0
    return cache_ctx


def advance_cache_context_step() -> tuple[int, int, int]:
    """
    Advance per-forward cache bookkeeping and return ``(call_idx, step_idx, branch_idx)``.
    """
    ...
    call_idx = int(getattr(cache_ctx, "fbcache_call_idx", 0))
    step_idx = call_idx // num_branches
    branch_idx = call_idx % num_branches

    cache_ctx.fbcache_call_idx = call_idx + 1
    cache_ctx.fbcache_current_step = step_idx
    cache_ctx.fbcache_current_branch_idx = branch_idx
    return call_idx, step_idx, branch_idx
...
def apply_prev_hidden_states_residual(
    ...
    """
    if mode == "multi":
        hidden_states_residual = get_buffer(_qualify_buffer_name("multi_hidden_states_residual", cache_key_prefix))
        ...
        if encoder_hidden_states is not None:
            enc_hidden_res = get_buffer(_qualify_buffer_name("multi_encoder_hidden_states_residual", cache_key_prefix))
        ...
        single_residual = get_buffer(_qualify_buffer_name("single_hidden_states_residual", cache_key_prefix))
...
@torch.compiler.disable
def can_apply_prev_hidden_states_residual(
    hidden_states: torch.Tensor,
    encoder_hidden_states: torch.Tensor | None = None,
    mode: str = "multi",
    cache_key_prefix: str | None = None,
    ...
    """
    Check whether cached residual tensors are shape-compatible with current states.
    """
    if mode == "multi":
        hidden_states_residual = get_buffer(_qualify_buffer_name("multi_hidden_states_residual", cache_key_prefix))
        if hidden_states_residual is None or hidden_states_residual.shape != hidden_states.shape:
            return False

        if encoder_hidden_states is not None:
            enc_hidden_res = get_buffer(_qualify_buffer_name("multi_encoder_hidden_states_residual", cache_key_prefix))
            if enc_hidden_res is None or enc_hidden_res.shape != encoder_hidden_states.shape:
                return False
    ...
    if mode == "single":
        single_residual = get_buffer(_qualify_buffer_name("single_hidden_states_residual", cache_key_prefix))
        return single_residual is not None and single_residual.shape == hidden_states.shape
...
def get_can_use_cache(
    ...
    """
    if mode == "multi":
        buffer_name = _qualify_buffer_name("first_multi_hidden_states_residual", cache_key_prefix)
    elif mode == "single":
        buffer_name = _qualify_buffer_name("first_single_hidden_states_residual", cache_key_prefix)
    else:
        raise ValueError(f"Unknown mode {mode}; expected 'multi' or 'single'")
...
def check_and_apply_cache(
    *,
    ...
    if allow_cache:
        ...
        can_use_cache = False
        diff = torch.tensor(threshold, device=first_residual.device)
    torch._dynamo.graph_break()

    if can_use_cache:
        residuals_compatible = can_apply_prev_hidden_states_residual(
            hidden_states,
            encoder_hidden_states,
            mode=mode,
            cache_key_prefix=cache_key_prefix,
        )
        if not residuals_compatible:
            can_use_cache = False
            if verbose:
                print(f"[{mode.upper()}] Cached residual shape mismatch, fallback to cache miss.")
    ...
    if mode == "multi":
        set_buffer(_qualify_buffer_name("first_multi_hidden_states_residual", cache_key_prefix), first_residual)
    else:
        set_buffer(_qualify_buffer_name("first_single_hidden_states_residual", cache_key_prefix), first_residual)
    ...
    if mode == "multi":
        ...
        set_buffer(_qualify_buffer_name("multi_hidden_states_residual", cache_key_prefix), hs_res)
        set_buffer(_qualify_buffer_name("multi_encoder_hidden_states_residual", cache_key_prefix), enc_res)
        return updated_h, updated_enc, threshold

    elif mode == "single":
        updated_cat_states, cat_res = result
        set_buffer(_qualify_buffer_name("single_hidden_states_residual", cache_key_prefix), cat_res)
